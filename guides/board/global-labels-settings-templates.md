...
Шаблоны досок — персональные и предустановленные.
...
> Шаблоны карточек и колонок удалены. Таблицы `card_templates` и `column_templates` остались в миграции, но не используются.
...
### Предустановленные шаблоны (frontend-only)
5 готовых шаблонов с предопределёнными колонками:
- **Kanban-доска** — Бэклог, В работе, На ревью, Тестирование, Готово
- **Домашние дела** — Надо сделать, В процессе, Покупки, Сделано
- **Спринт** — Спринт-бэклог, В разработке, Код-ревью, QA, Done
- **Воронка продаж** — Лиды, Контакт, Предложение, Переговоры, Закрыто
- **Контент-план** — Идеи, Написание, Редактура, Дизайн, Опубликовано

При выборе предустановленного шаблона: `createBoard()` → `createColumn()` × N.

### Frontend
- `TemplateManager.vue` — управление персональными шаблонами досок (вкладка "Шаблоны" в настройках доски)
- `CreateBoardModal.vue` — карточки предустановленных шаблонов + dropdown персональных + название + описание

### Тесты
- 4 domain теста (конструкторы, валидация BoardTemplate)
- 7 usecase тестов (CRUD, access control, create-from-template)
...
| Board templates | On-demand | Загружаются в модалках |
...
- `000015_templates.up.sql` — board_templates (+ неиспользуемые card_templates, column_templates)
