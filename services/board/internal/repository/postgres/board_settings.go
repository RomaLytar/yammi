...
func (r *BoardSettingsRepository) GetByBoardID(ctx context.Context, boardID string) (*domain.BoardSettings, error) {
	query := `
		SELECT board_id, use_board_labels_only, created_at, updated_at
		FROM board_settings
		WHERE board_id = $1
	`
	...
	err := r.db.QueryRowContext(ctx, query, boardID).Scan(
		&settings.BoardID, &settings.UseBoardLabelsOnly, &settings.CreatedAt, &settings.UpdatedAt,
	)
...
func (r *BoardSettingsRepository) Upsert(ctx context.Context, settings *domain.BoardSettings) error {
	query := `
		INSERT INTO board_settings (board_id, use_board_labels_only, created_at, updated_at)
		VALUES ($1, $2, $3, $4)
