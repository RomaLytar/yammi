...
export const options = {
  ...
  thresholds: {
    ...
    'latency_create_board_ms':      ['p(95)<2000'],
    'latency_create_card_ms':       ['p(95)<2000'],
    'latency_move_card_ms':         ['p(95)<2000'],
    'latency_add_member_ms':        ['p(95)<2000'],
    'latency_notifications_ms':     ['p(95)<2000'],
    'latency_notif_delivery_ms':    ['p(95)<20000'],
...
export function setup() {
  ...
  for (let i = 0; i < TOTAL_USERS; i++) {
    const email = `load2k-${i}-${ts}@yammi.io`;
    const name = `User ${i}`;
...
function createBoard(h, title) {
  ...
  const res = http.post(
    ...
    JSON.stringify({ title, description: 'Доска создана в load test 2k' }),
    h
  );
...
function updateCard(h, card, boardId, title) {
  ...
  const res = http.put(
    ...
    JSON.stringify({ title, description: 'Обновлено в load test 2k', board_id: boardId, version: card.version || 1 }),
    h
  );
